...
//                    for(y = 0; y < ig.game.entities.length; y++) {
//                        ig.game.entities[y].pos.y = ig.game.backgroundMaps[0].height * ig.game.backgroundMaps[0].tilesize - ig.game.entities[y].pos.y;
//                    }
//
...
//                    }
//
//                    for(y = 0; y < ig.game.backgroundMaps.length; y++) {
//                        this.extendMap(ig.game.backgroundMaps[y], mapData, 'n');
//                    }
//
...
//
//                    for(y = 0; y < ig.game.backgroundMaps.length; y++) {
//                        this.extendMap(ig.game.backgroundMaps[y], mapData, 's');
//                    }
//
//                    if(ig.game.collisionMap.data) {
...
//            }
            ...
            if(ig.game.backgroundMaps[0].width * ig.game.backgroundMaps[0].tilesize - ig.game.screen.x <= ig.system.width) {
                if(this.maps[this.currMap.y][this.currMap.x + 1]) {
                    ...
                    for(y = 0; y < ig.game.backgroundMaps.length; y++) {
                        this.extendMap(ig.game.backgroundMaps[y], mapData, 'e');
                    }
                ...
                if(ig.game.screen.x >= ig.game.backgroundMaps[0].tilesize) {
                    for(z = 0; z < ig.game.backgroundMaps.length; z++) {
                        for(y = 0; y < ig.game.backgroundMaps[z].data.length; y++) {
                            for(x = 0; x < ig.game.backgroundMaps[z].data[y].length; x++) {
                                ig.game.backgroundMaps[z].data[y].shift();
                            }
                        }
                        console.log(ig.game.backgroundMaps[0].data[0].length);

                        ig.game.backgroundMaps[z].width /= 2;
                    }

                    if(ig.game.collisionMap.data) {
                        for(y = 0; y < ig.game.collisionMap.data.length; y++) {
                            for(x = 0; x < ig.game.collisionMap.data[y].length; x++) {
                                ig.game.collisionMap.data[y].shift();
                            }
                        }

                        ig.game.collisionMap.width /= 2;
                    }

                    for(i = 0; i < ig.game.entities.length; i++) {
                        ig.game.entities[i].pos.x -= ig.game.backgroundMaps[0].width / 2 * ig.game.backgroundMaps[0].tilesize;
                    }

                    ig.game.screen.x -= ig.game.backgroundMaps[0].width / 2 * ig.game.backgroundMaps[0].tilesize;
                }
            }
...
//                    mapData = this.maps[this.currMap.y][this.currMap.x--];
//                    for(y = 0; y < ig.game.entities.length; y++) {
//                        ig.game.entities[y].pos.x = ig.game.backgroundMaps[0].width * ig.game.backgroundMaps[0].tilesize - ig.game.entities[y].pos.x;
//                    }
//
//                    for(i = 0; i < mapData.entities.length; i++) {
//                        entity = mapData.entities[i];
...
//
//                    for(y = 0; y < ig.game.backgroundMaps.length; y++) {
//                        this.extendMap(ig.game.backgroundMaps[y], mapData, 'w');
//                    }
//
//                    if(ig.game.collisionMap.data) {
