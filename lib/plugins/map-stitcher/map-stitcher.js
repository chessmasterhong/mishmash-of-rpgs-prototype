            ...
            if(ig.game.screen.y <= 0) {
                ...
                if(this.maps[this.currMap.y - 1]) {
                    ...
                    for(i = 0; i < ig.game.backgroundMaps.length; i++) {
                        this.extendMap(ig.game.backgroundMaps[i], nextMapData, 'n');
                    }

                    if(ig.game.collisionMap.data) {
                        this.extendMap(ig.game.collisionMap, nextMapData, 'n');
                    }
                }
            }
...
//
//                    for(i = 0; i < ig.game.backgroundMaps.length; i++) {
//                        this.extendMap(ig.game.backgroundMaps[i], nextMapData, 's');
//                    }
//
//                    if(ig.game.collisionMap.data) {
//                        this.extendMap(ig.game.collisionMap, nextMapData, 's');
//                    }
//                }
//            }
...
//            }
            ...
            if(ig.game.screen.x <= 0) {
                ...
                if(this.maps[this.currMap.y][this.currMap.x - 1]) {
                    ...
                    for(i = 0; i < ig.game.backgroundMaps.length; i++) {
                        this.extendMap(ig.game.backgroundMaps[i], nextMapData, 'w');
                    }

                    if(ig.game.collisionMap.data) {
                        this.extendMap(ig.game.collisionMap, nextMapData, 'w');
                    }
                }
            }
        },
